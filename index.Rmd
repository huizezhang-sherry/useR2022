...
* And we will see how wangling the data with cubble can be fun 
...
* Here we define the cube slightly different from a conventional cube. The three axes are Time, Site, and Variable.

* The axis *Site* encompasses the coordinates longitude and latitude and longitude and we make an assumption that an entity has a fixed location. 

* The axis **Variable** is used to represent multivariate information.

* We define the data in this way to avoid introducing hypercubes for multivariate information in the conventional cube. 

* With this cube, operations on multivariate spatio-temporal data can be thought of as slicing and dicing on the cube. 

* While the data cube model is conceptually convenient, for data wrangling the array structure may not sufficiently rich, for example, to wrangle special date time classes.

* Now I will demonstrate how cubble organises spatio-temporal data 
...
* The way cubble work is to organise spatial and temporal variables in two forms. The nested form for spatial variables and the long form for temporal variables.

* The nested cubble has each site in a row 

* Spatial variables like longitude, latitude, among others can be directly accessed for wrangling. temporal variables are all nested in a list column called `ts`. 

* In the long cubble, each row is identified by a combination of site and date, similar to `tsibble`.
...
* Use the normal dplyr syntax to work with two tables, you will need to repeat the filter step two times, one on each table. But with cubble, once a filter on the nested form, it will also update the data in the long form. 

* With dplyr syntax, there will be multiple intermediate objects created during the analysis, as we have seen before in the code comparison, but here, we can pipe through all the steps with only one object created. 
